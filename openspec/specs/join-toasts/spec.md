...
#### Scenario: Page refresh suppressed
- **WHEN** a named user disconnects and rejoins the same board within 5 seconds
- **THEN** no `user_joined` event SHALL be broadcast (treated as reconnect, not new arrival)
- **WHEN** more than 5 seconds elapse after disconnect before rejoin
- **THEN** a normal `user_joined` event SHALL be broadcast
